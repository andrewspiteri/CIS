...
- Preserve `AGENT-001`: CIS shall discover agent providers through public contracts rather than hard-coded module branches.
- Preserve `AGENT-002`: `cis agent providers` and `cis agent provider diagnose <provider>` shall report truthful provider availability.
- Preserve `AGENT-003`: The portable envelope contract shall remain available and become the input to direct execution.
- Preserve `AGENT-004`: `cis agent run <change-id> <task-id>` shall start only an eligible, digest-current task.
- Preserve `AGENT-005`: Every run shall target an explicitly registered workspace repository and contained working directory.
- Preserve `AGENT-006`: The controller shall set an execution mode and permission ceiling before launch.
- Preserve `AGENT-007`: A run shall expose a durable state machine and append-only event stream.
- Preserve `AGENT-008`: Codex support shall use the supported rich-client boundary for stateful execution.
- Preserve `AGENT-009`: Codex non-interactive execution shall remain available as an explicit bounded fallback.
- Preserve `AGENT-010`: Claude support shall use a supported headless or SDK boundary without parsing terminal presentation text.
- Preserve `AGENT-011`: Provider permission requests shall be mediated by CIS capability and policy.
- Preserve `AGENT-012`: `cis agent runs`, `show`, `cancel`, and `resume` shall manage observable runs without rewriting history.
- Preserve `AGENT-013`: Timeouts, interruption, and stale local locks shall have deterministic recovery.
- Preserve `AGENT-014`: Provider output shall be reconciled into the existing structured result and task-evidence model.
- Preserve `AGENT-015`: Canonical task state shall change only through an explicit CIS reconciliation step.
- Preserve `AGENT-016`: Derived run state shall live below `.cis/local/agents/runs/<run-id>/`.
- Preserve `AGENT-017`: Agent runs shall participate in CIS diagnostics, feedback, and cost accounting.
- Preserve `AGENT-018`: Credentials, provider configuration, prompts, output, and logs shall preserve repository privacy.
- Preserve `AGENT-019`: Process execution shall avoid shell interpolation and untrusted executable substitution.
- Preserve `AGENT-020`: Workspace policy shall define provider defaults and isolation without making a vendor mandatory.
- Preserve `AGENT-021`: Repository initialization shall seed provider-neutral instructions, skills, and a reference profile idempotently.
- Preserve `AGENT-022`: All agent commands shall provide stable human, JSON, and agent output suitable for a thin client.
- Preserve `AGENT-023`: Deterministic tests shall cover providers, lifecycle, security, recovery, and compatibility without requiring paid credentials.
- Preserve `AGENT-024`: Local smoke tests shall prove installed-client detection without requiring a live model request.
...
- [x] Every requirement, accepted impact, exclusion, and applicable task type is covered without hidden scope or unresolved disposition.
...
- [x] Validate task identities, coverage, dependencies, gates, lifecycle, and planned-versus-actual scope.

## Completion evidence

| Check | Command or artifact | Result | Notes |
| --- | --- | --- | --- |
| Scope and task disposition | `cis plan validate CIS-0002` | Passed | 24/24 requirements and 6/6 accepted impacts are covered; all nine actionable children have governed dispositions and no open decisions. |
| Planned versus actual | `cis verify diff CIS-0002` and final verification ledger | Passed | Agent coordination changed only the approved CIS repository; the CIS-0001 editor UI, cloud scheduling, detached daemons, and provider account management remain excluded. |
| CIS tool-usage snapshot | `.cis/local/feedback/tool-usage.jsonl` | Recorded | invocations=1086; failed=83; possibleTokenSavings=467162; ledgerDigest=sha256:9a85109401ec41fba5ff035fdbee33891dfef1c14774c388acd54379ec3aadd9 |
