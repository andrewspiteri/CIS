...
depends_on: ["WORK-030","WORK-040","WORK-080","WORK-090","WORK-130","WORK-140"]
approval_gate: none
...
| frontend-component | not-applicable | Frontend types: none | Component and accessibility execution |
| browser | not-applicable | Critical composed frontend journeys | Zero-retry passed execution with failure artifacts configured |
...
- [x] Trace TC-* identities into automated tests, refresh the catalogue, then run focused and affected suites, contract/migration checks, and coverage gates.

## Completion evidence

| Check | Command or artifact | Result | Notes |
| --- | --- | --- | --- |
| Exact case trace | `cis test trace CIS-0002 --run cis-0002-linux-20260828-r7` | Passed | All 24 `TC-AGENT-*` identities occur in passed reconciled executions; source-only annotations are insufficient. |
| Regression and coverage | R7 combined TRX and `.cis/local/testing/coverage/summary.json` | Passed | 448/448 tests passed; 29,602/37,663 lines covered (78.6%) against the repository 75% gate. |
| Client smoke | `cis agent provider diagnose codex` and `claude` | Passed | Codex executable/version was detected with authentication unavailable; missing Claude executable was reported truthfully without a live model call. |
| CIS tool-usage snapshot | `.cis/local/feedback/tool-usage.jsonl` | Recorded | invocations=1032; failed=83; possibleTokenSavings=425975; ledgerDigest=sha256:467261e7de14afa2bd59e331c327aa0b96c07d6268b7ec6e57e72070aa582c0d |
