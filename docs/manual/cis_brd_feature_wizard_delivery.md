...
names and content matches route to source; they do not establish that a requirement is satisfied. Ranking weights requirement terms, prioritizes matching methods and penalizes long files that mention many unrelated topics. The displayed excerpts prioritize relevant declarations and properties.
...
or **Exclude from this feature**. Owner, planned work and code references are prefilled as
suggestions. Edit them, then select **Save story decision**. New work does not require an
