...
```text
cis standards inventory [--repo <path>] [--target <target>...] [--stack <stack>...] [--status <status>] [--summary] [--format <human|json|agent>]
```
...
`--summary` returns status and counts without serializing each standard. It is intended for
workspace health projections; omit it for the full governance inventory.
