...
| GAP-PROC-008 | Pull-request and release CI | 5 | 5 | API, web, and infrastructure use repository-specific `/data` runners; docs uses GitHub-hosted Ubuntu. Strict required checks are enabled after green baselines. CIS and the Friends Todo repositories remain private under the current publication authority; this does not weaken private CI execution or release gating, and a later visibility change is an explicit publishing action rather than an implementation gap. | P0 | Closed |
