...
| GAP-TEST-020 | Test-time runtime log instrumentation | 4 | 4 | Workflow output streams to bounded attempt logs while the process runs; common credentials are redacted; partial timeout output survives; Testcontainers, browser, and Compose harnesses emit bounded suite diagnostics; reconciliation hashes and correlates both evidence classes to the run, attempt, suite, component, revision, and test identities. | P1 | Closed |
