...
public sealed partial class AgentService
{
    ...
    private static string BuildSolutionDesignInstruction(IReadOnlyList<string> targets, IReadOnlyList<ImplementationBundle> bundles)
    {
        var builder = new StringBuilder($$"""
            ...
            Add exactly one hidden data-only diagram model to the overall design: `<!-- cis:architecture-views`, newline, a JSON object, newline, `-->`. Shape: {"views":[{"id":"system-context","title":"System context","notes":"Evidence limitations and unresolved facts","nodes":[{"id":"client","label":"Customer application","layer":0,"status":"observed"},{"id":"api","label":"Product API","layer":1,"status":"observed"}],"edges":[{"from":"client","to":"api","label":"Authenticated product requests","status":"observed"}]}]}. Provide exactly four view IDs: system-context, component-topology, integration-trust, deployment-operations. Each view has 1–16 unique nodes and at most 24 directed relationships; group details into meaningful architectural boundaries and explain omitted detail in notes. Node IDs start with an ASCII letter and contain only ASCII letters, digits, underscores and hyphens (40 characters maximum). Layout layer is 0–3, from left to right; use it for readable diagrams, not to invent deployment tiers. Node labels are at most 100 characters, relationship labels 120, title 100, notes 800, all single-line. Every node and relationship has status exactly observed, unresolved or proposed. Every edge names two existing distinct nodes. No arbitrary SVG, HTML, Mermaid code, URLs or active content in this model. CIS renders native SVGs from it. Label unknown deployment as unresolved; do not fill it with an invented generic cloud topology. State trust boundaries in labels and explain them in the narrative.
