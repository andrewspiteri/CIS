...
public sealed class ArtifactDoctorCheck(ArtifactRetentionService service) : ICisRepositoryDoctorCheck
{
    ...
    {
        var result = service.Plan(context.RepositoryPath);
        ...
        var candidates = result.Entries.Count(item => item.Candidate);
        ...
        return [new("CIS-ARTIFACT-DOCTOR-002", "info", Name,
            ...
            result.Entries.Where(item => item.Candidate).Take(10).Select(item => item.Path).ToArray(),
