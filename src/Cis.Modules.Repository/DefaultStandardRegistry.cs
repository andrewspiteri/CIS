...
internal static class DefaultStandardRegistry
{
    ...
    [
        ...
        new(
            ...
            "Require portable deterministic security scanning, governed exceptions, redacted evidence, and advisory local-only model summaries.",
            ["security", "testing", "verification", "release"],
            [],
            ["docs/generic/standards/security-sast-standard.md", "docs/generic/standards/security-secret-scanning-standard.md"],
            HasImplementation,
            [
                Rule("SEC-TEST-001", "Repositories with production source MUST run portable blocking SAST independently of optional platform security features.", "Reconcile a Semgrep or equivalent result artifact and inspect the deterministic verdict."),
                Rule("SEC-TEST-002", "Every repository MUST scan its complete revision for secrets and MUST redact secret material from reports, logs, prompts, and retained artifacts.", "Run governed secret scanning and inspect normalized evidence for secret values."),
                Rule("SEC-TEST-003", "Dependency-bearing source MUST receive filesystem vulnerability scanning and Compose, Terraform, or equivalent deployment configuration MUST receive configuration scanning.", "Reconcile Trivy filesystem and configuration evidence for applicable components."),
                Rule("SEC-TEST-004", "Every releaseable container image MUST be scanned by immutable image identity before promotion; rebuilding after a passing scan invalidates that evidence.", "Compare the scanned digest or image ID with the promoted artifact."),
                Rule("SEC-TEST-005", "Externally reachable web or HTTP surfaces SHOULD receive DAST against a repository-managed representative runtime; authentication limits and unavailable credentials MUST remain explicit.", "Review ZAP or equivalent target, coverage, exclusions, and result evidence."),
                Rule("SEC-TEST-006", "A scanner process success without a readable declared result artifact MUST be classified as invalid evidence, never passed.", "Remove or corrupt a fixture result and run security reconciliation."),
                Rule("SEC-TEST-007", "Accepted findings MUST name the exact scanner and fingerprint, classification, bounded reason, owner, human approver, approval reference, and expiry.", "Run strict exception validation and inspect acceptance governance."),
                Rule("SEC-TEST-008", "Model-generated security summaries MUST use only normalized redacted findings, MUST stay local unless separately authorized, and MUST NOT change scanner severity, exceptions, or verdicts.", "Inspect summary metadata, prompt digest, local-only route, and deterministic verdict preservation."),
                Rule("SEC-TEST-009", "Security logs and results MUST be bounded, redacted, hashed, and correlated to scanner suite, workflow run, attempt, component, and repository revision.", "Inspect reconciled manifest and artifact metadata."),
                Rule("SEC-TEST-010", "Release verification MUST block on applicable unresolved critical or high findings, expired exceptions, invalid evidence, stale revision evidence, missing required security suites, or mutable scanner and external workflow-action references.", "Run release validation, compare the manifest revision and applicable suite states, and verify scanner digests and full action commit SHAs."),
            ]),
        new(
            "api-controller",
