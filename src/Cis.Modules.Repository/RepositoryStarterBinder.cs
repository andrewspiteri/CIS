...
internal sealed class RepositoryStarterBinder
{
    ...
    6. Read only enabled, non-sensitive diagnostic sources. Redact sensitive exports before CIS reads them.
    7. Run `cis learn collect` and `cis learn propose`; only a human may review, and only approved proposals may be promoted to canonical history.
    8. If repository initialization or command discovery fails, run `cis repo doctor` and follow evidence-backed fixes.
    """;
