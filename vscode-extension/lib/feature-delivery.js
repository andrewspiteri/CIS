...
function decisionDraft(result, story, drafts = {}) {
  ...
  const treatment = ['reuse', 'extend', 'new'].includes(story.treatment) ? story.treatment : '';
  return { treatment, owners: story.owners || [],
    plan: story.treatment !== 'unresolved' && story.remainingWork ? story.remainingWork
      : `Trace the related code for ${story.title}, confirm what can be reused, and describe the additions needed for these acceptance requirements:\n\n${(story.requirements || []).map(text => '- ' + text).join('\n')}`.slice(0, 4000),
    evidencePaths: treatment === 'new' ? '' : (story.evidenceIds || []).map(id => result.evidence?.find(e => e.id === id)).filter(Boolean).map(e => `${e.repositoryId}/${e.path}`).join('\n') };
}
...
function decisionEditor(result, story, drafts, busy) {
  ...
  return `<details class="delivery-decision" ${drafts[story.id] ? 'open' : ''}><summary>${story.review ? 'Review or change saved planning decision' : 'Resolve the delivery decision'}</summary>
    ...
    <div data-delivery-story="${h(story.id)}" ${drafts[story.id] ? 'data-delivery-edited="true"' : ''}><label>Planned treatment<select data-delivery-field="treatment"><option value="" ${!draft.treatment ? 'selected' : ''}>Choose after reviewing the evidence</option>${['new', 'extend', 'reuse', 'out-of-scope'].map(t => `<option value="${t}" ${draft.treatment === t ? 'selected' : ''}>${labels[t]}</option>`).join('')}</select></label>
...
function deliveryAssessment(result, { busy = false, drafts = {}, focus } = {}) {
  ...
  return `<section class="card feature-delivery-assessment"><h2>Existing capability and remaining work</h2>
    ...
    ${hasEvidence ? `<ol class="feature-story-list">${(result.stories || []).map(s => `<li ${focus === s.id ? 'data-delivery-focus' : ''}><details class="feature-story" ${drafts[s.id] || focus === s.id ? 'open' : ''}><summary>${h(s.title)} · ${h(s.reviewCurrent ? labels[s.review.treatment] + ' · Decision saved' : s.review ? 'Saved decision needs recheck' : assessmentLabels[s.assessmentState] || (s.assessmentState === 'proposed' ? labels[s.treatment] + ' · Proposal' : labels[s.treatment]) || labels.unresolved)}</summary>
      ...
      <p><strong>Existing capability:</strong> ${h(s.existingCapability === 'unknown' ? 'Not established from the selected code.' : s.existingCapability)}</p><p><strong>${s.assessmentState === 'proposed' ? 'Proposed work to review' : 'Next step'}:</strong> ${h(s.remainingWork)}</p>
      <p><strong>Repositories identified by the assessment:</strong> ${h(s.owners?.join(', ') || 'No owner established')}</p>
      <details><summary>Requirement checks (${s.requirements?.length || 0})</summary><p>Links identify related code to trace, not satisfied requirements. A requirement without a link has no supporting code established by this search; it may still exist elsewhere.</p><ol>${(s.requirementChecks?.length ? s.requirementChecks : (s.requirements || []).map(requirement => ({requirement,evidenceIds:[]}))).map(check => `<li><p>${h(check.requirement)}</p><p class="muted">${check.evidenceIds?.length ? 'Related code to inspect: ' + check.evidenceIds.map(id => result.evidence?.find(e => e.id === id)).filter(Boolean).map(e => `<button type="button" class="link" data-wizard-action="open-delivery-evidence" data-value="${h(e.id)}">${h(e.repositoryId)} · ${h(e.path)} ↗</button>`).join(', ') : 'No supporting code established'}</p></li>`).join('')}</ol></details>
      ${s.conflict ? `<p class="notice warning"><strong>Scope conflict:</strong> ${h(s.conflict)}</p>` : ''}
      <details><summary>Code leads and possible integration points</summary>${(s.evidenceIds || []).map(id => result.evidence?.find(e => e.id === id)).filter(Boolean).map(e => `<p><strong>${e.kind === 'capability-candidate' ? 'Capability candidate' : 'Possible integration point'}</strong> — ${h(e.summary || 'Review this code against the requirements before proposing reuse.')}</p><p><button type="button" class="link" data-wizard-action="open-delivery-evidence" data-value="${h(e.id)}">${h(e.repositoryId)} · ${h(e.path)} ↗</button></p><details><summary>View code excerpt</summary><pre>${h(e.excerpt)}</pre></details>`).join('') || '<p>No relevant excerpt was found in the bounded search. This does not prove that the capability is absent.</p>'}</details>
